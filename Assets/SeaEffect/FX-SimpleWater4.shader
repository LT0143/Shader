...
Properties {
	...
	_Transparency("Transparency of the whole water face", float) = 0.8
} 
...
CGINCLUDE
	...
	struct v2f_noGrab
	{
		...
		float3 bitanInterpolator : TEXCOORD5;
		float4 heightInfo : COLOR;
	};
	...
	{				
		...
		half4 edgeBlendFactors = half4(1.0, 0.0, 0.0, 0.0);
			...
			edgeBlendFactors = saturate(_InvFadeParemeter * (depth-i.screenPos.w));		//边缘圈白
		#endif	
	...
	o.tanInterpolator.xyz = UnityObjectToWorldDir(v.tangent.xyz);

	//half sign = v.tangent.w * unity_WorldTransformParam.w;
	half3 binormal = cross(o.normalInterpolator.xyz, o.tanInterpolator.xyz);
	o.bitanInterpolator = binormal;
		
		o.normalInterpolator.w = 1;//GetDistanceFadeout(o.screenPos.w, DISTANCE_SCALE); 

		o.heightInfo = v.color;
		
		return o;
	}

		half3 UnpackScaleNormalDIY(half4 packednormal, half bumpScale)
	{
		half3 normal;
...
//
	...
	inline float DecodeFloatRGBADIY(float4 rgba) {
	...
	{		
		//half3 worldNormal = PerPixelNormal(_BumpMap, i.bumpCoords, VERTEX_WORLD_NORMAL, PER_PIXEL_DISPLACE);
		//波浪
		half3 normalInTan = UnpackScaleNormalDIY(tex2D(_BumpMap, i.bumpCoords.xy)+ tex2D(_BumpMap, i.bumpCoords.zw), 1.0f); 
		half3 worldNormal = i.tanInterpolator.xyz * normalInTan.x + i.bitanInterpolator.xyz * normalInTan.y + i.normalInterpolator.xyz *  normalInTan.z;
		worldNormal = normalize(worldNormal);

		half3 viewVector = normalize(i.viewInterpolator.xyz);

		half4 distortOffset = half4(worldNormal.xz * REALTIME_DISTORTION * 1.0, 0.0, 0.0);//half4(worldNormal.xz * REALTIME_DISTORTION * 10.0, 0, 0);
		half4 screenWithOffset = i.screenPos + distortOffset;
...
/*			half depth = UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(i.screenPos)));
			...
			edgeBlendFactors = saturate(_InvFadeParemeter *DecodeFloatRGBADIY(i.heightInfo));
		#endif
			
		worldNormal.xz *= _FresnelScale;		
		half refl2Refr = Fresnel(viewVector, worldNormal, FRESNEL_BIAS, FRESNEL_POWER);
		//当拉伸没那么明显后，就发现倒影会一直延伸到陆地上。解决的方法依旧靠顶点色传过来的这个高度参数，越浅的地方，反射越小。
		...
		#endif
		//核心代码
		baseColor =  baseColor - edgeBlendFactors.x*0.3  + (spec * _SpecularColor)/(edgeBlendFactors.x+1) ;
		baseColor.a =  _Transparency * edgeBlendFactors.x* saturate(0.5 + refl2Refr * 1.0)*2;
		return baseColor;
		//float resultTest = DecodeFloatRGBADIY(i.heightInfo);
		//return resultTest;
	}	
